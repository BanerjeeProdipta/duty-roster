...
export function ShiftBadge({
	...
	const badge = (
		<div
			className={`inline-flex h-16 w-16 items-center justify-center rounded-xl border-2 font-bold text-xl shadow-lg transition-all hover:scale-105 hover:shadow-xl ${onChange ? "cursor-pointer ring-2 ring-transparent hover:ring-primary/50" : ""} ${SHIFT_STYLES[type]}`}
	...
	return (
		<DropdownMenu open={open} onOpenChange={setOpen}>
			...
			<DropdownMenuContent
				className="w-80 rounded-xl border-2 p-3 shadow-2xl"
				align="center"
				sideOffset={8}
			>
				...
				<DropdownMenuRadioGroup value={type} onValueChange={handleChange}>
					{SHIFT_OPTIONS.map((item) => (
						<DropdownMenuRadioItem
							key={item.value}
							value={item.value}
							className={`my-2 flex cursor-pointer items-center gap-4 rounded-lg py-4 pr-6 pl-4 text-lg transition-all ${
								type === item.value
